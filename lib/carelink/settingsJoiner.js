...
function convertLifecycleToNumber(e) {
  switch (e.lifecycle) {
    ...
    case 'end':
      return 1;
    default:
      throw except.ISE('Unknown lifecycle[%s], ts', e.lifecycle, e.deviceTime);
  }
}
...
/**
 * Combines together intermediate "settingsPart" events to produce the initial full-fledged "setting"
  ...
  return function(event) {
    ...
    function updateOrVerifyCommonFields(e) {
      for (var i = 0; i < commonFields.length; ++i) {
        ...
        if (currSchedule[field] == null) {
          currSchedule[field] = e[field];
        } else if (currSchedule[field] !== e[field]) {
          throw except.ISE('Mismatched field[%s], [%s] !== [%s]', field, currSchedule[field], e[field]);
        }
      }
      if (currSchedule.deviceTime == null) {
        currSchedule.deviceTime = e.deviceTime;
      } else if (currSchedule.deviceTime !== e.deviceTime) {
        throw except.ISE('Mismatched timestamps[%s] !== [%s]', currSchedule.deviceTime, e.deviceTime);
      }
    }

    function hasBasalSchedule() {
      return currSchedule.activeBasalSchedule !== undefined;
    }

    function hasBolusWizardSettings() {
      return currSchedule.carbRatio !== undefined;
    }

    function hasScheduleName(scheduleName) {
      return currSchedule.basalSchedules[scheduleName] !== undefined;
    }
    ...
    return {
      handle: function(e) {
        ...
        switch (e.subType) {
          case 'activeSchedule':
            ...
            currSchedule.activeBasalSchedule = e.scheduleName;
            break;
          case 'bolusWizard':
            ...
            updateOrVerifyCommonFields(e);
            _.assign(currSchedule, e.payload);
            currSchedule.units = e.units;
            break;
          case 'basalSchedule':
            ...
            updateOrVerifyCommonFields(e);
            currSchedule.basalSchedules[e.scheduleName] = e.payload;
            break;
          default:
      ...
      completed: function() {
        throw except.ISE('completed() called, but still aggregating a schedule. ts[%s]', currSchedule.deviceTime);
      }
    };
  }
}
  ...
  return function(observable) {
    return observable.link(function(observer) {
      ...
      var settingsHandlers = {
        ...
        basalSchedule: {
          onLifecycleEnd: function(e) {
            // This is called when there is a "CurrentBasalProfile" set of events in the middle of the csv feed
      ...
      var retVal = rx.Observer.create(
        function(e){
          ...
          } else if (handler.onLifecycleStart(e)) {
            // It's the start of the lifecycle, so we need to make sure it matches and emit an event if so
            observer.onNext(_.omit(currSettings, 'lifecycle'));
          }
        },
        function(error) {
